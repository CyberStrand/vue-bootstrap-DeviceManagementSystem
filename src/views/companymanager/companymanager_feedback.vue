...
export default {
  setup(){
    ...
    const fetchFeedback = () => {
      console.log("执行了fetchFeedback函数");
      fetch(`http://localhost:8080/company/feedbacks?pageNum=${pageNum.value}&pageSize=${pageSize.value}`, {
        method: 'GET',
        headers: apiHeaders,
      })
          .then(res => res.json())
      ...
      fetch(`http://localhost:8080/company/feedbacks?pageNum=${pageNum.value}&pageSize=${pageSize.value}`, {
        ...
        headers: apiHeaders,
          ...
          .then(res => {
            ...
            const tHeader = ['订单号', '公司号','反馈内容','用户ID'];
            const filterVal = ['orderId', 'companyId','feedbackContent','userId'];
